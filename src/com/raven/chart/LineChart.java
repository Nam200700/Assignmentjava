...
public class LineChart extends javax.swing.JPanel {
    ...
    public LineChart() {
        ...
        blankPlotChart.setBlankPlotChatRender(new BlankPlotChatRender() {
            ...
            public void renderSeries(BlankPlotChart chart, Graphics2D g2, SeriesSize size, int index, List<Path2D.Double> gra) {
                ...
                // Kiểm tra chiều dài của model.get(index).getValues() trước khi truy cập
                double[] values = model.get(index).getValues();

                for (int i = 0; i < legends.size(); i++) {
                    if (i < values.length) { // Kiểm tra nếu chỉ số i nhỏ hơn chiều dài của values
            ...
            public boolean mouseMoving(BlankPlotChart chart, MouseEvent evt, Graphics2D g2, SeriesSize size, int index) {
                ...
                double x = (size.getWidth() - totalSeriesWidth) / 2;
                for (int i = 0; i < legends.size(); i++) {
                    // Kiểm tra xem mảng values có đủ phần tử không
                    if (i < model.get(index).getValues().length) {
    ...
    public void addData(ModelChart data) {
        ...
        if (data.getValues() != null && data.getValues().length > 0) {
            ...
            System.out.println("Dữ liệu không hợp lệ hoặc trống.");
        }
    }
